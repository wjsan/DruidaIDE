Public Class GerenciamentoHardware
    ...
    Private Sub SaveTextBox()
        For i = 0 To txBoxNames.Count - 1
            ...
            file.WriteVar(item, txBoxValues(i))
        Next
    End Sub
    ...
    Private Sub SaveComboBox()
        For i = 0 To cbBoxNames.Count - 1
            ...
            file.WriteVar(item, cbBoxValues(i))
        Next
    End Sub
    ...
    Private Sub LoadTextBox()
        For i = 0 To txBoxNames.Count - 1
            Dim item = txBoxNames(i)
            txBoxValues(i) = file.ReadVar(item)
        Next
    End Sub
    ...
    Private Sub LoadComboBox()
        For i = 0 To cbBoxNames.Count - 1
            Dim item = cbBoxNames(i)
            cbBoxValues(i) = file.ReadVar(item)
        Next
    End Sub
    ...
    Public Function GetPinOut()
        Dim descr As String = "Descrição dos pinos do Arduino"
        Dim pinout As String = CurrentProjectCfgs.GetHeader(file.GetFilePath, descr)
        updateTextBoxValues()
        updateComboBoxValues()
    ...
    Public Function GetSetup()
        Dim setup As String = "void setupPins()" & vbTab & "//Procedimento que realiza a configuração dos pinos feita no Assistente de Configuração de Hardware" & vbCrLf
        setup &= "{" & vbCrLf
